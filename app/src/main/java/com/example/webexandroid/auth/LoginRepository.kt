...
class LoginRepository() {
        ...
        return Single.create<Boolean> { emitter ->
            oAuthAuthenticator.authorize(loginWebview, CompletionHandler { result ->
                ...
                if (result.error != null) {
                    Log.e("oautherror",result.error?.errorMessage)
        ...
        return Single.create<Boolean> { emitter ->
            jwtAuthenticator.authorize(token, CompletionHandler { result ->
                if (result.error != null) {
                    ...
                    Log.e("JWTresult",result.isSuccessful.toString())
                    emitter.onSuccess(result.isSuccessful)
                }
            })
        }.toObservable()
    ...
    fun loginWithToken(token: String, tokenAuthenticator: TokenAuthenticator): Observable<Boolean> {
        val expiryInSeconds = 60
        return Single.create<Boolean> { emitter ->
            tokenAuthenticator.authorize(token, expiryInSeconds, CompletionHandler { result ->
                if (result.error != null) {
                    ...
                    Log.e("TokenAuthResult",result.isSuccessful.toString())
                    emitter.onSuccess(result.isSuccessful)
                }
            })
        }.toObservable()
