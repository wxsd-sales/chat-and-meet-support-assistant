...
//    }
    ...
    fun postMessageDraftNew(target: String, text: String) {
        Log.e("messageComposer","inside")
        composerRepo.postMessageDraft(target, text).observeOn(AndroidSchedulers.mainThread()).subscribe({ result ->
            _postMessages.postValue(result)
        }, { error -> _postMessageError.postValue(error.message) }).autoDispose()
    }
