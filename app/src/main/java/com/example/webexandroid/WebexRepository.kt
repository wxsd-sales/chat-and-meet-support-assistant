...
import com.example.webexandroid.messaging.spaces.detail.MessageReceiveListener
...
class WebexRepository(val webex: Webex) : WebexUCLoginDelegate {
    ...
    fun messageObserver(): Observable<String> {
        return Single.create<String> { emitter ->
            ...
            webex.messages.setMessageObserver(object : MessageObserver {
                override fun onEvent(event: MessageObserver.MessageEvent) {
                    ...
                    when (event) {
                        is MessageObserver.MessageReceived -> {
                           // Log.e("webexmessagereceived",event.getMessage().toString())
                            emitter.onSuccess(event.getMessage().toString())
                        }
                    }
                }
            })
